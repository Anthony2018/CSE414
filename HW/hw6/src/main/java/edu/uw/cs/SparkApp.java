 ...
 |-- div5tailnum: integer (nullable = true)
   */
    ...
    public static JavaRDD<Row> QA(SparkSession spark, String dataFile) {
        ...
        // Use the Dataset SparkSQL interface
        d.createOrReplaceTempView("flights");
        Dataset<Row> r1 = spark.sql("SELECT DISTINCT destcityname FROM flights WHERE origincityname ='Seattle, WA'");
        //r1.show();
        rdd = r1.toJavaRDD();
        //rdd.foreach(cityString -> System.out.println(cityString));
        return rdd;
    }
    ...
    public static JavaRDD<Row> QB(SparkSession spark, String dataFile) {
        ...
        // Use the Dataset SparkSQL interface
        d.createOrReplaceTempView("flights");
//        Dataset<Row> r1 = spark.sql("SELECT origincityname as orgin_city, dayofmonth, COUNT(*) AS C\n" +
//                "FROM flights " +
//                "WHERE cancelled != 1\n" +
//                "GROUP BY origincityname,dayofmonth");
        Dataset<Row> r1 = d.groupBy(d.col("destcityname"),d.col("dayofmonth")).count();
        rdd = r1.toJavaRDD();
        //rdd.foreach(cityString -> System.out.println(cityString));
        return rdd;
    }
    ...
    public static JavaRDD<Row> QC(SparkSession spark, String dataFile) {
        ...
        Dataset<Row> r2 = d.select(d.col("destcityname"),d.col("arrdelayminutes")).groupBy("destcityname").avg();
